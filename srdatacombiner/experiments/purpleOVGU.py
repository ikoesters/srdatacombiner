...
class PurpleSausage(Experiments):
    def __init__(self) -> None:
        self.datafolder = (
            "../../data/25_02_10_PurpleOVGUv2_9.5mmBlade_1mpsSteps_1to10mps_5N"
        )
...
class Sensorprobe:
    def __init__(self, config: Experiments) -> None:
        self.config = config
        self.processor = Labjack_ovguFish
        self.sensor_kwargs = {}
        self.file_glob = "*dat"
...
if __name__ == "__main__":
    ...
    ds_flipped = ds[["vhead", "vmid", "vtail"]].sel(strvel=10, trial=slice(3, 5))
    ds_flip_corr = ds_flipped.sel(time=0) - (ds_flipped - ds_flipped.sel(time=0))
    for var in ["vhead", "vmid", "vtail"]:
        ds[var].loc[dict(strvel=10, trial=slice(3, 5))] = ds_flip_corr[var]
    # %%
